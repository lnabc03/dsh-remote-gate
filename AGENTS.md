...
| `start.mjs` | 进程编排 + 面板宿主：拉起面板/dsh/网关/隧道，单窗口日志加前缀 | dsh 崩溃自动重启（5 次×3s）；网关/frpc/cloudflared **意外**退出团灭，ssh 退出 3s 重拨（约束 7）；面板保存触发的重启靠 gen 代际区分，别绕过 stopGate/stopTunnel 直接 kill；**热重启必须 await `waitExit` 等旧进程退出释放端口再拉新**——killTree 是异步的（Windows 走 taskkill），立即 spawn 新网关会 EADDRINUSE 秒退，被 exit 回调误判为意外崩溃 → 团灭（踩过）；cf 临时域名由 startCfProc 抓日志（`extractCfUrl`）→ 拼令牌打印 + 推面板；dsh 直接 spawn node bin.js（约束 6），杀派生树用 `taskkill /T`；`--no-ui` 关闭面板；**面板应用窗口生死与本进程耦合**：shutdown() 主动 `killPanelBrowser`，意外死亡由 `watchdog.mjs` 兜底；控制台 stdin 按回车 = 重开面板窗口（start.bat 用 `start /min` 经 **PowerShell 宿主**拉起 node——pwsh 优先、powershell 回退，PS 启动时禁用 QuickEdit，点选不冻结、回车一次即生效；bat 自身 `exit /b` 闪退，保持纯 ASCII）；**启动崩溃可见性**：首行必打 node 版本横幅（无横幅 = node 没起来），uncaughtException/unhandledRejection/main 异常一律经 `fatalCrash` 追加写 `start-crash.log`（已 gitignore）再 shutdown——bat 最小化窗口下早期崩溃本来无声无息；浏览器/看门狗的 spawn 必须带 `'error'` 监听（异步 spawn 失败无监听器 = uncaughtException 炸掉整个启动器）；**主入口判定用 `samePath`**（realpath + Windows 大小写兼容），勿改回 `argv[1] === import.meta.url` 严格相等——经目录联接点启动时两者不等，main() 会被静默跳过、进程零输出秒退（踩过） |
