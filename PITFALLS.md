...
| 杀控制台重启后只起 node 不起面板；偶发冷启动面板也不出窗（进程全起、无可见窗口） | chromium 系 `--app` 窗口首次创建偶发不显示，被强杀后重启尤其易现；清 `profile.exit_type`（=Crashed）崩溃标记无效——不是崩溃恢复挡的 | `openInBrowser` 拉窗后**异步验证**：12s 内「有进程但无可见窗口」→ 再 spawn 一次，经浏览器单例 handoff 把窗口拉出（实测 ~1s）；窗口已可见则跳过 spawn，避免 Enter 重开/重复启动产生双窗口 |
