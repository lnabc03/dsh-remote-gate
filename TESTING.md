...
- [x] `npm start -- --mode cf` → `config.json` 出现 `mode:"cf"`；日志出现 `[cf]` 前缀，无 `[frpc]`/`[ssh]`（已实测可用，2026-08）
- [x] 数秒后 `[start]` 打印 `登录链接: https://<随机串>.trycloudflare.com/?t=…`，无 cloudflared 的边框刷屏
- [x] 手机打开该链接 → 302 → 进入 DSH，可正常对话、流式输出正常
