...
> 纯逻辑已由自动化测试覆盖（`npm test`，当前 88 项）；本清单只列必须要有真实服务器 / 真机 / 真网络才能验的项，逐条打勾即可。
...
- [ ] 双击 `start.bat`：bat 窗口一闪即关，node 控制台**最小化在任务栏**（不消失），可见窗口只有面板一个；点开最小化控制台能看到全部日志
- [ ] 控制台里按回车：重新打开面板应用窗口（面板被误关后的找回口子）
- [ ] 模拟控制台意外死亡（`taskkill /F /PID <node>` 或直接 X 掉控制台窗口）：数秒内面板应用窗口被看门狗自动关闭，不静默挂着；其余派生进程（dsh/网关/隧道）随控制台退出
- [ ] 关闭后立即双击 `start.bat` 重启：node 起来后，面板窗口要么直接弹出、要么约 12s 内被自动重拉弹出（chromium `--app` 窗口偶发不出窗，`openInBrowser` 会验证并重拉，不该需要手动回车）
- [ ] 面板窗口已打开时再按回车（或重复启动）：不产生第二个面板窗口，日志出现「控制面板窗口已在运行，未重复打开」
